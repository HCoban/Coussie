...
import { receiveMoreReviews, clearReviews, addNewReview, clearSingleReview, ReviewConstants } from '../actions/review_actions';
...
const RestaurantMiddleware = ({dispatch, getState}) => next => action => {
  ...
  const createReviewSuccess = (data) => dispatch(addNewReview(data));
  ...
  switch (action.type) {
    ...
    case RestaurantConstants.CREATE_REVIEW:
      createReview(action.review, createReviewSuccess);
      return next(action);
